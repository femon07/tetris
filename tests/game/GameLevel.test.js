...
describe('ゲームレベル管理', () => {
  ...
  describe('getDropInterval', () => {
    ...
    test('レベル5のドロップ間隔が正しい', () => {
      ...
      game.gameState.dropInterval = game.gameState.getDropInterval();
      expect(game.dropInterval).toBe(600);
    });
    
    test('レベル10のドロップ間隔が正しい', () => {
      // レベル10に変更してテスト
      game.gameState.level = 10;
      game.gameState.dropInterval = game.gameState.getDropInterval();
      expect(game.dropInterval).toBe(100);
    });
    
    test('レベルが上限を超えた場合、最大レベルの間隔を返す', () => {
      // レベル20に変更してテスト
      game.gameState.level = 20;
      game.gameState.dropInterval = game.gameState.getDropInterval();
      expect(game.dropInterval).toBe(100); // レベル10以降は100ms固定
    });
  });
  
  describe('checkLevelUp', () => {
    test('ライン数が10未満の場合、レベルアップしない', () => {
      game.gameState.lines = 9;
      const result = game.gameState.checkLevelUp();
      expect(result).toBe(false);
      expect(game.level).toBe(1);
    });
    
    test('ライン数が10の場合、レベル2にアップする', () => {
      game.gameState.lines = 10;
      const result = game.gameState.checkLevelUp();
    ...
    test('ライン数が19の場合、レベル2のまま', () => {
      game.gameState.lines = 19;
      game.gameState.checkLevelUp();
      expect(game.level).toBe(2);
    });
    
    test('ライン数が20の場合、レベル3にアップする', () => {
      game.gameState.lines = 20;
      const result = game.gameState.checkLevelUp();
      expect(result).toBe(true);
      expect(game.level).toBe(3);
    });
    
    test('レベルが既に高い場合、適切にレベルアップする', () => {
      game.gameState.level = 15;
      game.gameState.lines = 150; // レベル16相当のライン数
      const result = game.gameState.checkLevelUp();
      expect(result).toBe(true);
      expect(game.level).toBe(16);
      
      // 再度チェックしてもレベルは変わらない
      const result2 = game.gameState.checkLevelUp();
  ...
  describe('dropPiece とレベルアップの統合', () => {
    test('ラインを10個消すとレベルが上がる', () => {
      ...
      game.gameState.addLines(linesCleared);
  ...
  describe('スコア計算とレベルの関係', () => {
    test('レベルが高いほどスコアが高くなる', () => {
      // レベル1で1ライン消した場合のスコア
      const scoreLevel1 = game.gameState.scoreCalculator.calculateLineScore(1, 1);
      
      // レベル2で1ライン消した場合のスコア
      const scoreLevel2 = game.gameState.scoreCalculator.calculateLineScore(1, 2);
    ...
    test('複数ラインを消した場合のスコア計算', () => {
      ...
      const score4Lines = game.gameState.scoreCalculator.calculateLineScore(4, 1);
      
      // レベル2で4ライン消した場合のスコア
      const score4LinesLevel2 = game.gameState.scoreCalculator.calculateLineScore(4, 2);
